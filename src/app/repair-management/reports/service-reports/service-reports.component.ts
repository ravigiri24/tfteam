...
export class ServiceReportsComponent  implements OnInit {
  ...
  generateExcel() {
    ...
    const element = document.getElementById('contentReportService');
...
totalProfit:any=0
  createReport() {
    ...
    this.totalNetPay=0
    this.totalProfit=0
    let jobList=this.allDetails?.jobList?.filter((f:any)=>f.repair_type=='SERVICE')
    jobList?.forEach((job: any) => {
      ...
      obj.totalBillService=job?.totalBillService||0
      ...
      this.totalProfit=Number(this.totalProfit)+ Number(obj.ProfitService)
  ...
  generatePDF() {
    ...
    const content: any = document.getElementById('contentReportService');
