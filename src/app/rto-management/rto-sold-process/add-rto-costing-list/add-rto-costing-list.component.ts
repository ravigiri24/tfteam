...
import { AddRtoCostingComponent } from '../add-rto-costing/add-rto-costing.component';
@Component({
  selector: 'app-add-rto-costing-list',
  templateUrl: './add-rto-costing-list.component.html',
  styleUrls: ['./add-rto-costing-list.component.scss'],
})
export class AddRtoCostingListComponent  implements OnInit {
    ...
    async showModal(dataUpdate: any = null) {
      const modal = await this.modalCtrl.create({
        component: AddRtoCostingComponent,
        componentProps: {
    ...
    totalAmount = 0;
    getList() {
      let obj: any = this.share.getListObj('rto_expense_costing', false, [], true);
      obj.tractor_id = this.tarctor_id;
      this.share.showLoading('Loading...');
      this.api.postapi('getRTOExpenseDetailsById', obj).subscribe(
        (res: any) => {
    ...
    expenseTypeList: any = [];
    getExpense() {
      let obj: any = this.share.getListObj('expense_rto', false, [], true);
      obj.tractor_id = this.tarctor_id;
