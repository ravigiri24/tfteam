...
@Component({
  selector: 'app-view-ndp-sell-price',
  templateUrl: './view-ndp-sell-price.component.html',
  styleUrls: ['./view-ndp-sell-price.component.scss'],
})
export class ViewNdpSellPriceComponent  implements OnInit {
  tractorDetails: any;
  ...
  getSavedJobList() {
    ...
    this.api.postapi('getJobsByTractor', obj).subscribe(
     ...
     let findInTag=this.tagsArray.findIndex((tag:any)=>tag.type=='JOB_ARRAY')
     if(findInTag>-1){
   this.tagsArray.splice(findInTag,1)
     }

      if(!this.savedJobList?.length){
    this.tagsArray.push({tag:"No Job Card Included",type:"JOB_ARRAY"})
      }
      let isJobCardOpen=false
        this.savedJobList?.forEach((job: any) => {
    if(job?.isCompleted==0){
       isJobCardOpen=true
          ...
          this.getReduceList(job?.id, objectCosting);
           ...
           let findInTagO=this.tagsArray.findIndex((tag:any)=>tag.type=='JOB_OPEN')
     if(findInTagO>-1){
   this.tagsArray.splice(findInTagO,1)
     }
         if(isJobCardOpen){
    this.tagsArray.push({tag:"Job Card Is Currently Open",type:"JOB_OPEN"})
      }
        setTimeout(() => {
    ...
    this.api.postapi('getExtraExpenseDetailsById', obj).subscribe(
      (res: any) => {
        ...
        this.otherExpenseCost = this.totalAmountOther;
              ...
              if(!this.otherExpenseCost || this.otherExpenseCost==0){
    this.tagsArray.push({tag:"Other Expense May Not Included",type:"OTHER_EXPENSE"})
      }
        //    this.share.spinner.dismiss();
      },
      (error: any) => {}
  ...
  calculateAmountMaintainance() {
    ...
    this.maintainanaceCost =
      ...
      (this.expenseServiceCost || 0) -
      (this.reduceItemTotalAmount || 0);
      let findInTag=this.tagsArray.findIndex((tag:any)=>tag.type=='MAINTAINENCE_COST')
         if(findInTag>-1){
   this.tagsArray.splice(findInTag,1)
     }
    
      if(this.maintainanaceCost==0||!this.maintainanaceCost){
    this.tagsArray.push({tag:"Maintainance Cost May Not Included",type:"MAINTAINENCE_COST"})
      }
  }
  ...
  getTractorDetails(msg: any = 'Loading...') {
    ...
    this.api.postapi('getTractorByIdAllDetails', obj).subscribe(
      (res: any) => {
        ...
        if(this.purchaseCost==0||!this.purchaseCost){
   this.tagsArray.push({tag:"Purchase Price May Not Included",type:"PURCHASE_PRICE"})
        }
        this.rtoCost = Number(this.tractorDetails?.rto_cost || 0);
        this.insuranceCost = Number(this.tractorDetails?.insurance_cost || 0);
           if(this.rtoCost==0||!this.rtoCost){
   this.tagsArray.push({tag:"RTO Cost May Not Included",type:"RTO_COST"})
        }
            if(this.insuranceCost==0||!this.insuranceCost){
   this.tagsArray.push({tag:"Insurance Cost May Not Included",type:"INSURANCE_COST"})
        }
    ...
    this.api.postapi('getTransportDetailsById', obj).subscribe(
      (res: any) => {
        ...
        if (this.logisticData?.length) {
          ...
          if(this.logisticCost==0||!this.logisticCost){
      this.tagsArray.push({tag:"Logistic Cost May Not Included",type:"LOGISTIC_COST"})
        }
